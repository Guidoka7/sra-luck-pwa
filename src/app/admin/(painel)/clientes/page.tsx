...
import { Plus, Search, UserRound, LayoutGrid, List } from "lucide-react";
  ...
  return (
    <div className="space-y-5 pb-8">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-[0.65rem] font-semibold uppercase tracking-[0.18em] text-rose">Gestão</p>
          <h1 className="mt-1 text-2xl font-semibold text-burgundy sm:text-3xl">Clientes</h1>
          <p className="mt-1 text-sm text-clay/50">Perfil, crédito e parcelas em um único espaço.</p>
        </div>
      ...
      <Card className="flex flex-wrap items-center gap-2.5 p-3">
        <div className="relative min-w-[220px] flex-1">
          ...
          <Input value={busca} onChange={(e) => setBusca(e.target.value)} placeholder="Buscar nome, CPF ou telefone…" className="pl-10" />
        </div>
        <div className="flex items-center gap-1 rounded-full border border-rose/10 bg-cream p-1">
          <button onClick={() => setVisualizacao("lista")} className={`rounded-full p-2 ${visualizacao === "lista" ? "bg-burgundy text-cream" : "text-clay/40"}`} aria-label="Lista"><List className="h-3.5 w-3.5" /></button>
          <button onClick={() => setVisualizacao("cards")} className={`rounded-full p-2 ${visualizacao === "cards" ? "bg-burgundy text-cream" : "text-clay/40"}`} aria-label="Cards"><LayoutGrid className="h-3.5 w-3.5" /></button>
        </div>
      </Card>
      ...
      {carregando ? <SkeletonCards count={6} /> : erro ? (
        ...
        <Card className="p-10 text-center text-sm text-clay/45">Nenhuma cliente encontrada.</Card>
      ) : visualizacao === "lista" ? (
        <Card className="overflow-hidden p-0">
          <div className="divide-y divide-rose/5">
            {filtradas.map((c) => (
              <button key={c.id} type="button" onClick={() => setModal(c)} className="grid w-full grid-cols-[minmax(0,1.5fr)_minmax(120px,.7fr)_minmax(120px,.7fr)_auto] items-center gap-3 px-4 py-3.5 text-left transition hover:bg-blush/20 sm:px-5">
                <div className="flex min-w-0 items-center gap-3">
                  <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-blush text-burgundy"><UserRound className="h-4 w-4" /></span>
                  <span className="min-w-0"><span className="block truncate text-sm font-semibold text-burgundy">{c.nome_completo}</span><span className="block text-[0.68rem] text-clay/40">{formatarCpf(c.cpf)}</span></span>
                </div>
                <span className="hidden text-xs text-clay/55 sm:block">{c.procedimento || "Sem procedimento"}</span>
                <span className="text-xs text-clay/55">{statusPagamento(c)}</span>
