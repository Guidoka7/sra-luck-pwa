...
function SectionTitle({ icon: Icon, title, description }: { icon: React.ElementType; title: string; description: string }) {
  return (
    <div className="flex min-w-0 items-center gap-2.5">
      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-burgundy/8 text-burgundy dark:bg-white/[0.07] dark:text-rose">
        <Icon className="h-4 w-4" />
      </span>
      <div className="min-w-0">
        <h2 className="text-sm font-semibold text-burgundy dark:text-pearl">{title}</h2>
        <p className="truncate text-[11px] leading-4 text-clay/45 dark:text-pearl/35">{description}</p>
      </div>
    </div>
  );
}
...
function FieldHint({ children }: { children: React.ReactNode }) {
  return <p className="mt-1 text-[10px] leading-4 text-clay/40 dark:text-pearl/30">{children}</p>;
}
...
export default function ConfiguracoesPage() {
  ...
  async function carregar(force = false) {
    ...
    if (cacheValido) {
      aplicarConfiguracoes(cached!.configuracoes!);
      setCarregando(false);
    } else {
  ...
  async function alternarBloqueioAgenda() {
    ...
    try {
      const res = await fetch("/api/admin/configuracoes", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ agendaLiberacaoFinanceiraBloqueada: novoValor }),
      });
      const data = await res.json();
  ...
  return (
    <div className="space-y-3 pb-6">
      <PageHeader
        ...
        description="Identidade, agenda, pagamentos e contatos em uma visão compacta."
        actions={
          <span className="inline-flex items-center gap-1.5 rounded-full border border-burgundy/10 bg-white/60 px-2.5 py-1.5 text-[10px] font-medium text-clay/55 shadow-sm dark:border-white/10 dark:bg-white/[0.04] dark:text-pearl/45">
            <CheckCircle2 className="h-3 w-3 text-success" /> Atualizado {atualizado}
          </span>
        }
      />
      ...
      {carregando || !dados ? <SkeletonRows count={3} /> : (
        <>
          <div className="grid gap-2 sm:grid-cols-3">
            <div className={`${MINI} flex min-h-0 items-center gap-2.5 px-3 py-2.5`}>
              <Building2 className="h-4 w-4 shrink-0 text-burgundy/60 dark:text-rose" />
              <div className="min-w-0"><p className="text-[9px] font-semibold uppercase tracking-[0.16em] text-clay/40 dark:text-pearl/30">Empresa</p><p className="truncate text-xs font-semibold text-burgundy dark:text-pearl">{nomeClinica || "Não definida"}</p></div>
            </div>
            <div className={`${MINI} flex min-h-0 items-center gap-2.5 px-3 py-2.5`}>
              <WalletCards className="h-4 w-4 shrink-0 text-gold" />
              <div className="min-w-0"><p className="text-[9px] font-semibold uppercase tracking-[0.16em] text-clay/40 dark:text-pearl/30">Orçamento mensal</p><p className="text-xs font-semibold text-burgundy dark:text-pearl">R$ {Number(meta || 0).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</p></div>
            </div>
            <div className={`${MINI} flex min-h-0 items-center gap-2.5 px-3 py-2.5`}>
              <CalendarClock className={`h-4 w-4 shrink-0 ${agendaBloqueada ? "text-alert" : "text-success"}`} />
              <div className="min-w-0"><p className="text-[9px] font-semibold uppercase tracking-[0.16em] text-clay/40 dark:text-pearl/30">Agenda financeira</p><p className={`text-xs font-semibold ${agendaBloqueada ? "text-alert" : "text-success"}`}>{agendaBloqueada ? "Bloqueada" : "Aberta"}</p></div>
            </div>
          </div>
          ...
          <div className="grid gap-3 lg:grid-cols-[1.15fr_.85fr]">
            <section className={`${PANEL} overflow-hidden`}>
              ...
              <form onSubmit={salvar} className="p-4 sm:p-5">
                <div className="grid gap-3 sm:grid-cols-2">
                  <div><Label htmlFor="nomeClinica">Nome da empresa</Label><Input id="nomeClinica" value={nomeClinica} onChange={e => setNomeClinica(e.target.value)} placeholder="Nome exibido no painel" className={CAMPO_INPUT} /><FieldHint>Usado na experiência administrativa.</FieldHint></div>
                  <div><Label htmlFor="metaMensal">Limite orçamentário mensal</Label><Input id="metaMensal" type="number" min="0" step="0.01" value={meta} onChange={e => setMeta(e.target.value)} placeholder="Ex.: 100000" className={CAMPO_INPUT} /><FieldHint>Referência para liberações financeiras.</FieldHint></div>
                ...
                <div className="mt-3 flex justify-end"><Button type="submit" size="sm" loading={salvando}><Save className="h-3.5 w-3.5" /> Salvar identidade</Button></div>
              </form>
            </section>
            ...
            <section className={`${PANEL} overflow-hidden`}>
              ...
              <div className="p-4 sm:p-5">
                <div className={`flex h-full flex-col justify-between gap-3 rounded-xl border p-3 ${agendaBloqueada ? "border-alert/15 bg-alert/5" : "border-success/15 bg-success/5"}`}>
                  <div className="flex min-w-0 items-start gap-2.5">
                    <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${agendaBloqueada ? "bg-alert/10 text-alert" : "bg-success/10 text-success"}`}>{agendaBloqueada ? <Lock className="h-4 w-4" /> : <ShieldCheck className="h-4 w-4" />}</span>
                  ...
                  <Button type="button" size="sm" variant={agendaBloqueada ? "secondary" : "danger"} loading={salvandoAgenda} onClick={alternarBloqueioAgenda} className="w-full">{agendaBloqueada ? <><Unlock className="h-3.5 w-3.5" /> Reabrir agenda</> : <><Lock className="h-3.5 w-3.5" /> Bloquear agenda</>}</Button>
                </div>
                <p className="mt-2 text-[9px] leading-4 text-clay/35 dark:text-pearl/25">Não altera datas já confirmadas nem a agenda de termos cirúrgicos.</p>
              </div>
            </section>
          </div>
            ...
            <form onSubmit={salvarPagamento} className="p-4 sm:p-5">
              <div className="grid gap-4 lg:grid-cols-[1fr_220px]">
                ...
                <div>
                  ...
                  <div className="mt-1 flex min-h-[170px] flex-col items-center justify-center rounded-xl border border-dashed border-burgundy/15 bg-burgundy/[0.025] p-3 text-center transition hover:border-gold/50 dark:border-white/10 dark:bg-white/[0.025]" onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); selecionarArquivoQrCode(e.dataTransfer.files?.[0] ?? null); }}>
                    ...
                    {pixQrCode ? <>
                      <div className="relative rounded-xl border border-burgundy/10 bg-white p-1.5 shadow-sm dark:border-white/10"><img src={pixQrCode} alt="Pré-visualização do QR Code PIX" className="h-24 w-24 rounded-lg object-contain" /></div>
                      <p className="mt-1.5 text-[10px] font-medium text-success">QR Code carregado</p>
                      <div className="mt-2 flex gap-1.5"><Button type="button" variant="ghost" size="sm" onClick={() => inputArquivoRef.current?.click()}><UploadCloud className="h-3.5 w-3.5" /> Trocar</Button><Button type="button" variant="danger" size="sm" onClick={() => setPixQrCode("")}><Trash2 className="h-3.5 w-3.5" /> Remover</Button></div>
                    </> : <button type="button" onClick={() => inputArquivoRef.current?.click()} className="flex flex-col items-center gap-1.5"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-gold/10 text-gold"><QrCode className="h-4 w-4" /></span><span className="text-xs font-semibold text-burgundy dark:text-pearl">Adicionar QR Code</span><span className="text-[10px] text-clay/40 dark:text-pearl/35">Clique ou arraste · PNG, JPG ou WEBP · até 1.5MB</span><span className="mt-0.5 inline-flex items-center gap-1 text-[9px] font-medium text-burgundy/50 dark:text-pearl/35"><ImagePlus className="h-3 w-3" /> Pré-visualização instantânea</span></button>}
                  </div>
                </div>
              </div>
              <div className="mt-3 flex items-center justify-end gap-3 border-t border-burgundy/8 pt-3 dark:border-white/8"><p className="mr-auto hidden text-[10px] text-clay/35 dark:text-pearl/25 sm:block">As alterações aparecem para a cliente após o salvamento.</p><Button type="submit" size="sm" loading={salvandoPagamento}><Save className="h-3.5 w-3.5" /> Salvar pagamento e contato</Button></div>
            </form>
          </section>
        </>
      )}
