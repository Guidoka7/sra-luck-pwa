  ...
  return (
    <div className="admin-shell admin-compact min-h-screen bg-bloom">
      ...
      <div className="relative flex items-center justify-between border-b border-white/60 bg-white/80 px-3 py-2 shadow-sm backdrop-blur-xl lg:hidden">
        ...
        <div className="flex items-center gap-1"><ThemeToggle compact /><button onClick={() => setMenuAberto(true)} aria-label="Abrir menu" className="rounded-lg border border-rose/15 bg-white/70 p-1.5 text-burgundy transition-colors hover:bg-blush"><Menu className="h-4 w-4" /></button></div>
      </div>
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-[1720px] gap-2.5 px-2 pb-2 lg:px-3 lg:pt-3">
        <aside className={cn("fixed inset-y-0 left-0 z-50 flex w-64 max-w-[86vw] flex-col rounded-r-xl border-r border-white/60 bg-white/82 px-3 py-3 text-clay shadow-[0_30px_100px_-40px_rgba(122,38,50,0.28)] backdrop-blur-2xl transition-transform duration-300 ease-out", "lg:sticky lg:top-3 lg:h-[calc(100vh-1.5rem)] lg:translate-x-0 lg:rounded-xl lg:border lg:border-white/70", menuAberto ? "translate-x-0" : "-translate-x-full lg:translate-x-0")}>
          <div>
            <div className="mb-3 flex items-center justify-between"><div className="min-w-0 flex-1"><div className="mb-1.5 flex h-8 w-8 items-center justify-center rounded-lg border border-rose/15 bg-blush/45"><LogoMark className="h-4 w-4" /></div><Wordmark maxWidth={150} /><p className="mt-1 text-[9px] uppercase tracking-[0.24em] text-burgundy/45">Painel premium</p></div><button onClick={() => setMenuAberto(false)} aria-label="Fechar menu" className="rounded-lg p-1 text-clay/45 hover:bg-blush/70 hover:text-burgundy lg:hidden"><X className="h-3.5 w-3.5" /></button></div>
            <nav className="mt-3 space-y-2.5">{Array.from(new Set(NAV.map((item) => item.group))).map((group) => <div key={group}><p className="mb-1 px-1.5 text-[9px] uppercase tracking-[0.2em] text-burgundy/38">{group}</p><div className="space-y-0.5">{NAV.filter((item) => item.group === group).map((item) => { const ativo = pathname.startsWith(item.href); return <Link key={item.href} href={item.href} prefetch onMouseEnter={() => prefetchAdminTab(router, item.href)} onFocus={() => prefetchAdminTab(router, item.href)} className={cn("group flex items-center gap-2 rounded-lg px-2 py-1.5 text-[13px] transition-all duration-200", ativo ? "bg-burgundy text-pearl shadow-soft" : "text-clay/78 hover:bg-white/70 hover:text-burgundy")}><span className={cn("flex h-7 w-7 items-center justify-center rounded-md transition-colors", ativo ? "bg-white/16 text-pearl" : "bg-blush/60 text-burgundy group-hover:bg-blush")}><item.icon className="h-3 w-3" /></span><div className="min-w-0 flex-1"><p className="truncate font-medium">{item.label}</p><p className={cn("truncate text-[9px]", ativo ? "text-pearl/72" : "text-burgundy/40")}>{item.group}</p></div></Link>; })}</div></div>)}</nav>
          </div>
          <div className="space-y-1.5"><ThemeToggle /><button onClick={sair} className="flex w-full items-center justify-center gap-1.5 rounded-lg border border-rose/12 bg-white/72 px-2.5 py-2 text-[13px] text-burgundy/78 transition-colors duration-200 hover:bg-blush/70 hover:text-burgundy"><LogOut className="h-3 w-3" /> Sair</button></div>
        </aside>
