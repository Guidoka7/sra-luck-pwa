    ...
    <div className="admin-shell admin-compact min-h-screen bg-bloom">
      ...
      <div className="relative flex items-center justify-between border-b border-white/60 bg-white/80 px-4 py-3 shadow-sm backdrop-blur-xl lg:hidden">
        <div className="flex items-center gap-2.5">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl border border-white/70 bg-white text-burgundy shadow-card">
            <LogoMark className="h-5 w-5" />
          </div>
          <div>
            <span className="block text-[10px] font-semibold uppercase tracking-[0.24em] text-burgundy/55">Admin</span>
            <span className="block text-base text-burgundy">Painel Executivo</span>
          </div>
        </div>
        <div className="flex items-center gap-1.5">
          <ThemeToggle compact />
          <button onClick={() => setMenuAberto(true)} aria-label="Abrir menu" className="rounded-xl border border-rose/15 bg-white/70 p-2 text-burgundy transition-colors hover:bg-blush">
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-[1720px] gap-4 px-3 pb-4 lg:px-4 lg:pt-4">
        <aside
          className={cn(
            "fixed inset-y-0 left-0 z-50 flex w-72 max-w-[86vw] flex-col rounded-r-2xl border-r border-white/60 bg-white/82 px-4 py-4 text-clay shadow-[0_30px_100px_-40px_rgba(122,38,50,0.28)] backdrop-blur-2xl transition-transform duration-300 ease-out",
            "lg:sticky lg:top-4 lg:h-[calc(100vh-2rem)] lg:translate-x-0 lg:rounded-2xl lg:border lg:border-white/70",
        ...
        >
          <div>
            <div className="mb-5 flex items-center justify-between">
              <div className="min-w-0 flex-1">
                <div className="mb-2 flex h-9 w-9 items-center justify-center rounded-xl border border-rose/15 bg-blush/45">
                  <LogoMark className="h-5 w-5" />
                </div>
                <Wordmark maxWidth={175} />
                <p className="mt-2 text-[10px] uppercase tracking-[0.28em] text-burgundy/45">Painel premium</p>
              </div>
              <button onClick={() => setMenuAberto(false)} aria-label="Fechar menu" className="rounded-lg p-1.5 text-clay/45 hover:bg-blush/70 hover:text-burgundy lg:hidden">
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <nav className="mt-5 space-y-4">
              {Array.from(new Set(NAV.map((item) => item.group))).map((group) => (
                <div key={group}>
                  <p className="mb-1.5 px-2 text-[10px] uppercase tracking-[0.22em] text-burgundy/38">{group}</p>
                  <div className="space-y-1">
                    {NAV.filter((item) => item.group === group).map((item) => {
                      ...
                      return (
                        ...
                        >
                          <span className={cn("flex h-8 w-8 items-center justify-center rounded-lg transition-colors", ativo ? "bg-white/16 text-pearl" : "bg-blush/60 text-burgundy group-hover:bg-blush")}>
                            <item.icon className="h-3.5 w-3.5" />
                          </span>
                          <div className="min-w-0 flex-1">
                            <p className="truncate font-medium">{item.label}</p>
                            <p className={cn("truncate text-[10px]", ativo ? "text-pearl/72" : "text-burgundy/40")}>{item.group}</p>
                          </div>
                        </Link>
                      );
                    })}
          ...
          <div className="space-y-2">
            <ThemeToggle />
            <button onClick={sair} className="flex w-full items-center justify-center gap-2 rounded-xl border border-rose/12 bg-white/72 px-3 py-2.5 text-sm text-burgundy/78 transition-colors duration-200 hover:bg-blush/70 hover:text-burgundy">
