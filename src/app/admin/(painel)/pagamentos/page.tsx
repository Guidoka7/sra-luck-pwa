...
function PagamentosConteudo() {
  ...
  const boletosFiltrados = useMemo(() => {
    ...
    const lista = !termo
      ? boletos
      : boletos.filter((b) => b.clientes?.nome_completo.toLowerCase().includes(termo) || b.clientes?.cpf.includes(termo.replace(/\D/g, "")));
  ...
  const grupos = useMemo(() => {
    ...
    for (const boleto of boletosFiltrados) {
      if (!mapa.has(boleto.cliente_id)) mapa.set(boleto.cliente_id, {
  ...
  async function marcarSelecionadosComoPagos() {
    ...
    try {
      const res = await fetch("/api/admin/boletos/lote", {
        ...
        body: JSON.stringify({ ids: Array.from(selecionados), acao: "marcar_pago" }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.erro ?? "Não foi possível marcar as parcelas como pagas.");
      toast.success(`${Number(data.total ?? 0)} parcela(s) marcada(s) como paga(s).`);
      await carregar(true);
    } catch (error) {
  ...
  return (
    <div className="space-y-5 pb-8">
      ...
      <Panel className="p-4 sm:p-5">
        <SectionHeading title="Pagamentos" description="Selecione as parcelas em aberto para marcar como pagas. A edição de valores e vencimentos fica na aba Parcelas." />
        ...
        {selecionados.size > 0 && (
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2.5 rounded-2xl border border-burgundy/15 bg-burgundy/[0.06] px-3.5 py-2.5">
            ...
            <Button size="sm" loading={processando} onClick={marcarSelecionadosComoPagos}>
              <CheckCircle2 className="h-3.5 w-3.5" /> Marcar selecionadas como pagas
            </Button>
          </div>
        )}
        ...
        {carregando ? <SkeletonRows count={5} /> : grupos.length === 0 ? (
          ...
          <div className="space-y-3">{grupos.map((grupo) => (
            <ClientePagamentos key={grupo.clienteId} grupo={grupo} selecionados={selecionados} onSelecionar={alternarSelecionado} onAbrir={setBoletoAtivo} />
          ))}</div>
        )}
      </Panel>
...
function Resumo({ label, valor, destaque, alerta }: { label: string; valor: number; destaque?: boolean; alerta?: boolean }) {
  return <div className={cn("rounded-2xl border px-3 py-3", destaque ? "border-gold/25 bg-gold/5" : alerta ? "border-alert/20 bg-alert/5" : "border-white/10 bg-white/[0.025]")}>
...
function ClientePagamentos({ grupo, selecionados, onSelecionar, onAbrir }: { grupo: GrupoCliente; selecionados: Set<string>; onSelecionar: (id: string) => void; onAbrir: (b: Boleto) => void }) {
  ...
  return <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.02]">
    <div className="flex flex-wrap items-center justify-between gap-2 border-b border-white/8 px-3.5 py-3">
      <div className="min-w-0"><p className="truncate text-sm font-semibold text-burgundy">{grupo.nome}</p><p className="text-[0.62rem] text-clay/40">CPF {formatarCpf(grupo.cpf)} · {grupo.parcelas.length} parcela(s)</p></div>
    ...
    <div className="divide-y divide-white/7">{grupo.parcelas.map((boleto) => {
      ...
      const aberto = boleto.status !== "pago";
      ...
      return <div key={boleto.id} className={cn("px-3.5 py-3", vencida && "bg-alert/[0.035]", aguardandoConfirmacao && "bg-gold/[0.035]")}>
        <div className="flex flex-wrap items-center gap-2.5">
          {aberto && <input type="checkbox" checked={selecionados.has(boleto.id)} onChange={() => onSelecionar(boleto.id)} className="h-4 w-4 accent-burgundy" aria-label={`Selecionar parcela ${boleto.numero_parcela}`} />}
        ...
        {aguardandoConfirmacao && <div className="mt-2.5 grid gap-2 rounded-xl border border-gold/15 bg-gold/[0.035] p-2.5 sm:grid-cols-[1fr_auto] sm:items-center">
          <div><p className="text-[0.62rem] font-semibold text-gold">Comprovante enviado</p><p className="mt-0.5 text-[0.58rem] text-clay/45">Abra o comprovante para conferir antes de confirmar.</p></div>
          <Button size="sm" onClick={() => onAbrir(boleto)}><ReceiptText className="h-3.5 w-3.5" /> Conferir comprovante</Button>
        </div>}
        {boleto.status !== "pendente_confirmacao" && boleto.comprovante_url && <div className="mt-2 flex justify-end"><button type="button" onClick={() => onAbrir(boleto)} className="inline-flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-[0.58rem] text-burgundy/55 hover:bg-blush hover:text-burgundy"><ReceiptText className="h-3 w-3" /> Ver comprovante</button></div>}
      </div>;
    })}</div>
  </div>;
}
...
function ModalValidarComprovante({ boleto, onClose, onResolvido }: { boleto: Boleto; onClose: () => void; onResolvido: () => void }) {
  ...
  return <Portal><div className="fixed inset-0 z-50 flex items-center justify-center bg-burgundy-dark/55 p-3 backdrop-blur-md" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
    <div className="max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-[24px] border border-white/10 bg-[#1b181b] p-4 text-pearl shadow-2xl sm:p-5">
      <div className="flex items-start justify-between gap-3 border-b border-white/8 pb-3">
        <div><p className="text-[0.62rem] uppercase tracking-[0.16em] text-gold">Conferência</p><h2 className="mt-1 text-lg font-semibold text-rose">Parcela {boleto.numero_parcela}/{boleto.total_parcelas}</h2><p className="text-xs text-pearl/45">{boleto.clientes?.nome_completo ?? "Cliente"} · vencimento {formatarData(boleto.data_vencimento)}</p></div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-2">
        <InfoBox label="Valor da parcela" valor={formatarMoeda(boleto.valor)} />
        <InfoBox label="Status" valor={STATUS_BOLETO_LABEL[boleto.status]} />
      </div>
      {boleto.comprovante_url ? <a href={boleto.comprovante_url} target="_blank" rel="noreferrer" className="mt-3 flex items-center justify-between rounded-xl border border-rose/15 bg-rose/[0.05] px-3 py-3 text-xs text-rose"><span className="flex items-center gap-2"><ReceiptText className="h-4 w-4" /> Abrir comprovante para conferência</span><ExternalLink className="h-4 w-4" /></a> : <div className="mt-3 rounded-xl border border-alert/20 bg-alert/[0.05] px-3 py-3 text-xs text-alert">Nenhum comprovante foi anexado a esta parcela.</div>}
      <div className="mt-3"><label className="mb-1 block text-[0.58rem] uppercase tracking-[0.12em] text-pearl/35">Observação (opcional)</label><Textarea value={observacoes} onChange={(e) => setObservacoes(e.target.value)} placeholder="Observação da conferência…" /></div>
...
function InfoBox({ label, valor }: { label: string; valor: string }) {
  return <div className="rounded-xl border border-white/8 bg-white/[0.025] p-3"><p className="text-[0.56rem] uppercase tracking-[0.13em] text-pearl/35">{label}</p><p className="mt-1 text-sm font-bold text-pearl">{valor}</p></div>;
}
