...
function AgendaClienteConteudo() {
  ...
  const [agendaLiberada, setAgendaLiberada] = useState(false); const [podeAgendar, setPodeAgendar] = useState(false); const [statusRevisao, setStatusRevisao] = useState<StatusRevisaoFinanceira | null>(null); const [observacaoRevisao, setObservacaoRevisao] = useState<string | null>(null); const [porcentagemPagamento, setPorcentagemPagamento] = useState<number | null>(null); const [quantidadeParcelas, setQuantidadeParcelas] = useState<number | null>(null); const [config, setConfig] = useState<ConfigPublica | null>(null); const [destacarCardLiberacao, setDestacarCardLiberacao] = useState(false);
  ...
  async function carregar(silencioso = false) {
    ...
    if (previsaoAnterior !== undefined && novaPrevisao !== previsaoAnterior && novaPrevisao) { toast.success(previsaoAnterior ? "📅 Sua data da cirurgia foi atualizada. Confira em \"Minha Agenda\"." : "📅 Sua data da cirurgia já está disponível em \"Minha Agenda\"."); if (!previsaoAnterior) { setDestacarCardLiberacao(true); setTimeout(() => setDestacarCardLiberacao(false), 4000); } }
