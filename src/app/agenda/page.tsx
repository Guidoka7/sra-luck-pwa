...
function AgendaClienteConteudo() {
  ...
  async function carregar(silencioso = false) {
    ...
    if (resBoletos.ok) { const boletosData = await resBoletos.json(); const temBoletos = (boletosData.boletos ?? []).length > 0; setAgendaLiberada(temBoletos && Boolean(boletosData.agenda_liberada)); setStatusRevisao(temBoletos ? boletosData.status_revisao_financeira ?? null : null); setPorcentagemPagamento(temBoletos ? boletosData.porcentagem_pagamento : null); setQuantidadeParcelas(boletosData.quantidade_parcelas ?? null); } else { setAgendaLiberada(false); setStatusRevisao(null); setPorcentagemPagamento(null); setQuantidadeParcelas(null); }
    setCarregando(false);
  }
  ...
  async function escolherData(dataId: string, horario: string) {
    setConfirmando(true);
    try { const res = await fetch("/api/cliente/agendar", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ dataId, horario }) }); const resultado = await res.json(); if (!res.ok) { toast.error(resultado.erro ?? "Não foi possível confirmar essa data."); return; } setCelebrando(resultado.data); }
