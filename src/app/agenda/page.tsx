...
export default function AgendaClientePage() {
  ...
  const [podeAgendar, setPodeAgendar] = useState(false); const [agendaLiberada, setAgendaLiberada] = useState(false); const [statusRevisao, setStatusRevisao] = useState<StatusRevisaoFinanceira | null>(null); const [porcentagemPagamento, setPorcentagemPagamento] = useState<number | null>(null); const [quantidadeParcelas, setQuantidadeParcelas] = useState<number | null>(null); const [config, setConfig] = useState<ConfigPublica | null>(null); const [destacarCardLiberacao, setDestacarCardLiberacao] = useState(false);
  const agendaLiberadaRef = useRef(false); const previsaoRef = useRef<string | null | undefined>(undefined);
  ...
  const tourStage = agendamentoAtivo || agendamentoConcluido ? "cirurgia" : agendaLiberada ? "agenda" : "financeiro"; const percentualContrato = percentualNecessario(quantidadeParcelas); const parcelasNecessarias = quantidadeParcelas ? Math.ceil((quantidadeParcelas * percentualContrato) / 100) : null;
  const agendaAtual = agendamentoAtivo ?? agendamentoConcluido;
  ...
  return <main data-tour-root data-tour-stage={tourStage} data-tour-required-percentage={percentualContrato} data-tour-required-installments={parcelasNecessarias ?? ""} className="client-app min-h-[100dvh] bg-bloom px-4 pb-[max(env(safe-area-inset-bottom),1rem)] pt-[max(env(safe-area-inset-top),0.75rem)] sm:px-6 sm:pt-6 sm:pb-8">
    ...
    <div className="mobile-app-frame mx-auto w-full max-w-2xl sm:max-w-[30rem]">
      ...
      <div data-tour="jornada" className="mb-4"><JourneyTracker percentualPagamento={porcentagemPagamento ?? 0} percentualAtingido={podeAgendar} statusRevisao={statusRevisao} agendada={Boolean(agendaAtual)} previsaoLiberacaoFinanceira={agendaAtual?.previsaoLiberacaoFinanceira ?? null} /></div>
      ...
      {aba === "cirurgia" && (agendamentoAtivo ? <div data-tour="previsao" className="flex flex-col gap-4 animate-fadeUp"><SolicitarLiberacaoFinanceira ativo={agendaLiberada || statusRevisao === "aprovada"} /><CardPrevisaoLiberacao previsaoLiberacaoFinanceira={agendamentoAtivo.previsaoLiberacaoFinanceira} dataAssinatura={agendamentoAtivo.data} destacar={destacarCardLiberacao} /><Card className="p-3.5"><div className="flex items-center gap-2 text-success"><CheckCircle2 className="h-4 w-4" /><span className="text-xs font-semibold">Assinatura dos termos agendada</span></div><p className="mt-1 text-sm text-clay/60">{agendamentoAtivo.data.split("-").reverse().join("/")}{agendamentoAtivo.horario ? ` às ${agendamentoAtivo.horario}` : ""}</p></Card></div> : agendamentoConcluido ? <div data-tour="previsao" className="flex flex-col gap-4 animate-fadeUp"><Card className="border border-success/15 bg-success/[0.04] p-4"><div className="flex items-center gap-2 text-success"><CheckCircle2 className="h-4 w-4" /><span className="text-xs font-semibold uppercase tracking-label">Termos assinados</span></div><h2 className="mt-2 font-heading text-lg font-semibold text-burgundy">Assinatura confirmada</h2><p className="mt-1 text-sm leading-relaxed text-clay/60">Sua assinatura foi confirmada em {agendamentoConcluido.data.split("-").reverse().join("/")}{agendamentoConcluido.horario ? ` às ${agendamentoConcluido.horario}` : ""}. A previsão de liberação financeira foi definida para {agendamentoConcluido.previsaoLiberacaoFinanceira?.split("-").reverse().join("/") ?? "—"}.</p></Card><SolicitarLiberacaoFinanceira ativo={agendaLiberada || statusRevisao === "aprovada"} /><CardPrevisaoLiberacao previsaoLiberacaoFinanceira={agendamentoConcluido.previsaoLiberacaoFinanceira} dataAssinatura={agendamentoConcluido.data} destacar={destacarCardLiberacao} /></div> : <div className="flex flex-col gap-4"><RegrasLiberacao quantidadeParcelas={quantidadeParcelas} /><motion.div data-tour="agenda" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1, ease: [0.22, 1, 0.36, 1] }}><Card className="p-3.5 sm:p-4">{datas.length === 0 ? <p className="p-6 text-center text-sm text-clay/50">Ainda não há datas disponíveis no momento. Fale com a nossa equipe para saber mais.</p> : <CalendarioAgendamento datas={datas} onConfirmar={escolherData} confirmando={confirmando} bloqueado={!agendaLiberada} />}</Card></motion.div></div>)}
    </div>
