...
export async function GET() {
  ...
  const { data, error } = await supabase.from("clientes").select(`
    ...
    valor_contrato, taxa_administrativa_percentual, status_cirurgia,
  ...
  for (const b of boletos ?? []) {
    const atual = resumo.get(b.cliente_id) ?? { total: 0, pagos: 0 };
    atual.total += 1;
    if (b.status === "pago") atual.pagos += 1;
    resumo.set(b.cliente_id, atual);
  }
  return NextResponse.json({ clientes: (data ?? []).map((cliente) => {
    const r = resumo.get(cliente.id);
    return { ...cliente, porcentagem_pagamento: r?.total ? Math.round((r.pagos / r.total) * 1000) / 10 : null, parcelas_pagas: r?.pagos ?? null, parcelas_total: r?.total ?? null };
  }) });
}
