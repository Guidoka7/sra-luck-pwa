...
function data90Dias(iso: string | null | undefined) {
  if (!iso) return null;
...
export async function GET() {
  ...
  const solicitacoes = (data ?? []).map((item: any) => {
    const dataTermos = item.agendamentos?.datas?.data ?? null;
    return { ...item, data_termos: dataTermos, previsao_sugerida: data90Dias(dataTermos) };
  });
...
export async function PATCH(req: NextRequest) {
  ...
  if (dataLiberacao && atual.agendamento_id) {
    const { error: erroAgendamento } = await supabase
      .from("agendamentos")
      .update({ previsao_liberacao_financeira: dataLiberacao })
      .eq("id", atual.agendamento_id);
  ...
  await supabase.from("logs_alteracoes").insert({
    ...
    acao: dataLiberacao ? "programou_liberacao_financeira" : `solicitacao_liberacao_${status ?? "atualizada"}`,
    entidade: "solicitacoes_liberacao_financeira",
    entidade_id: id,
    detalhes: { status: status ?? atual.status, dataLiberacaoFinanceira: dataLiberacao, observacao },
  });
