...
export async function PATCH(req: NextRequest) {
  ...
  if (!id || !status || !STATUS.includes(status as (typeof STATUS)[number])) {
    return NextResponse.json({ erro: "Solicitação ou status inválido." }, { status: 400 });
  }
  ...
  const { data: atual, error: erroAtual } = await supabase
    ...
    .eq("id", id)
    .select("id, cliente_id, status, forma_custeio, saldo_restante, taxa_cartao, total_com_taxa")
    .single();
  if (erroAtual) return NextResponse.json({ erro: erroAtual.message }, { status: 500 });

  await supabase.from("logs_alteracoes").insert({
    usuario: user.email ?? "admin",
    acao: `solicitacao_liberacao_${status}`,
