...
function evento(usuario: string, detalhes: Record<string, unknown>) { return { em: new Date().toISOString(), usuario, acao: "Vinculação confirmada", detalhes }; }
...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  if (!clienteId || !carneId || !boletoId) return NextResponse.json({ erro: "Cliente, carnê e boleto existente são obrigatórios." }, { status: 400 });
  ...
  if (importacao.status_vinculacao === "vinculado" || importacao.boleto_vinculado_id) return NextResponse.json({ erro: "Esta importação já está vinculada." }, { status: 409 });
  ...
  if (!boleto || boleto.cliente_id !== clienteId || boleto.carne_id !== carneId) return NextResponse.json({ erro: "O boleto não pertence à cliente e ao carnê selecionados." }, { status: 400 });
  ...
  if (conflito) return NextResponse.json({ erro: "Este boleto existente já está vinculado a outra importação." }, { status: 409 });

  const historicoAtual = Array.isArray(importacao.historico) ? importacao.historico : [];
  const historico = [...historicoAtual, evento(user.id, { status_anterior: importacao.status_vinculacao, cliente_anterior: importacao.cliente_vinculado_id, cliente_novo: clienteId, carne_anterior: importacao.carne_vinculado_id, carne_novo: carneId, boleto_anterior: importacao.boleto_vinculado_id, boleto_novo: boletoId, pontuacao: importacao.pontuacao_confianca ?? 0 })];
  const { data, error } = await supabase.from("importacoes_boletos").update({ cliente_id: clienteId, carne_id: carneId, boleto_id: boletoId, cliente_vinculado_id: clienteId, carne_vinculado_id: carneId, boleto_vinculado_id: boletoId, status_vinculacao: "vinculado", status: "vinculado", historico }).eq("id", params.id).select("*").single();
