...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  if (!clienteId || !carneId || !boletoId) return NextResponse.json({ erro: "Selecione cliente, carnê e boleto existente." }, { status: 400 });
  ...
  const historico = [...historicoAtual, { em: new Date().toISOString(), usuario: user.id, acao: "Vinculação manual confirmada", detalhes: { cliente_novo: clienteId, carne_novo: carneId, boleto_novo: boletoId, pontuacao: importacao.pontuacao_confianca ?? 0 } }];
