...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  ...
  let candidatos: any[] = [];
  if (ids.length) {
    const result = await supabase.from("boletos").select("id,cliente_id,carne_id,numero_parcela,total_parcelas,valor,data_vencimento,status,instituicao_financeira,identificador_externo,origem_boleto,cliente:clientes!boletos_cliente_id_fkey(id,nome_completo,cpf,telefone),carne:carnes!boletos_carne_id_fkey(id,identificador_externo,instituicao_financeira,quantidade_parcelas)").in("id", ids);
