...
export async function GET(req: NextRequest) {
  ...
  let query = supabase.from("importacoes_boletos").select(`id,arquivo_nome,cliente_id,carne_id,boleto_id,cliente_sugerido_id,carne_sugerido_id,boleto_sugerido_id,cliente_vinculado_id,carne_vinculado_id,boleto_vinculado_id,instituicao_financeira,nosso_numero,numero_documento,identificador_externo,nome_pagador_extraido,cpf_pagador_extraido,valor_extraido,vencimento_extraido,numero_parcela,pontuacao_confianca,nivel_confianca,status_vinculacao,status,analise_detalhada,created_at,updated_at,cliente:clientes!importacoes_boletos_cliente_id_fkey(id,nome_completo,cpf,telefone),cliente_sugerido:clientes!importacoes_boletos_cliente_sugerido_id_fkey(id,nome_completo,cpf,telefone),cliente_vinculado:clientes!importacoes_boletos_cliente_vinculado_id_fkey(id,nome_completo,cpf,telefone),carnes(id,identificador_externo,instituicao_financeira,quantidade_parcelas),boletos(id,numero_parcela,total_parcelas,valor,data_vencimento,status)`).order("created_at", { ascending: false }).limit(300);
