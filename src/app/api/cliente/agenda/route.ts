...
export async function GET(req: NextRequest) {
  ...
  const { data: agendamentoAtivo } = await supabase.from("agendamentos")
    .select("id, data_id, status, horario_termos, termos_assinados_em, previsao_liberacao_financeira, datas(data)")
  ...
  return NextResponse.json({
    ...
    agendamentoAtivo: agendamentoAtivo ? { id: agendamentoAtivo.id, data: (agendamentoAtivo as any).datas?.data, horario: agendamentoAtivo.horario_termos ? String(agendamentoAtivo.horario_termos).slice(0, 5) : null, termosAssinadosEm: agendamentoAtivo.termos_assinados_em ?? null, previsaoLiberacaoFinanceira: agendamentoAtivo.previsao_liberacao_financeira ?? null } : null,
