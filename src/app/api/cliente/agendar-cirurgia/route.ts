...
export async function POST(req: NextRequest) {
  ...
  const { data: agendamento } = await supabase.from("agendamentos").select("id, data_id, previsao_liberacao_financeira").eq("cliente_id", cliente.id).eq("status", "confirmado").maybeSingle();
