...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data: boleto } = await supabase
    .from("boletos")
    .select("cliente_id, comprovante_url")
    .eq("id", params.id)
    .single();

  if (!boleto || boleto.cliente_id !== sessao.clienteId || !boleto.comprovante_url) {
    return NextResponse.json({ erro: "Comprovante não encontrado." }, { status: 404 });
  }

  const { data, error } = await supabase.storage
    .from(BUCKET)
    .createSignedUrl(boleto.comprovante_url, 60 * 5); // 5 minutos
