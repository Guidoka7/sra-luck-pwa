...
export async function POST(req: NextRequest) {
  ...
  if (!formaCusteio || !FORMAS.includes(formaCusteio)) {
    return NextResponse.json({ erro: "Escolha uma forma de custeio válida." }, { status: 400 });
  }
  ...
  if (cliente.status_revisao_financeira !== "aprovada") {
    return NextResponse.json({ erro: "A solicitação só pode ser enviada após a liberação da agenda." }, { status: 409 });
  }
  ...
  const saldoRestante = (boletos ?? [])
    .filter((b: { status: string }) => b.status !== "pago")
    .reduce((total: number, b: { valor: number }) => total + Number(b.valor || 0), 0);
  ...
  if (existente) {
    return NextResponse.json({ erro: "Sua solicitação já está em análise.", solicitacao: existente }, { status: 409 });
  }
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      saldo_restante: Math.round(saldoRestante * 100) / 100,
      taxa_cartao: Math.round(taxaCartao * 100) / 100,
      total_com_taxa: Math.round(totalComTaxa * 100) / 100,
      status: "pendente",
    })
    .select("id, forma_custeio, saldo_restante, taxa_cartao, total_com_taxa, status, created_at")
    .single();
  ...
  await supabase.from("logs_alteracoes").insert({
    ...
    entidade_id: data.id,
    detalhes: { cliente: cliente.nome_completo, formaCusteio, saldoRestante, taxaCartao, totalComTaxa },
  });
