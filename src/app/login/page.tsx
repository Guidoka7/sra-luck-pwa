...
export default function LoginClientePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(data.erro ?? "Não foi possível confirmar seus dados.");
        return;
      }
      router.push("/agenda");
      router.refresh();
  ...
  return (
    <main className="client-login min-h-screen flex items-center justify-center bg-bloom px-6 pt-[calc(env(safe-area-inset-top)+2rem)] pb-[calc(env(safe-area-inset-bottom)+2rem)]">
      <div className="relative flex w-full max-w-md flex-col items-center animate-fadeUp">
        ...
        <div className="mb-3 flex h-14 w-14 items-center justify-center rounded-[24px] border border-white/70 bg-white/90 shadow-card dark:border-white/10 dark:bg-white/[0.055]">
        ...
        <Card className="w-full p-8 sm:p-9">
          ...
          <p className="mb-7 text-center text-sm leading-6 text-clay/60">
            Entre com seus dados para ver sua agenda.
          </p>
        ...
        <p className="mt-6 text-center text-xs text-clay/40">
          Seus dados de acesso foram cadastrados pela nossa equipe.
          <br />
