...
import { ModalClienteLiberacaoFinanceira } from "@/components/admin/ModalClienteLiberacaoFinanceira";

const DIAS = ["D", "S", "T", "Q", "Q", "S", "S"];
type Estado = "verde" | "amarelo" | "vermelho" | "cinza" | "passado";
type AcaoData = "liberar" | "fechar" | null;
interface Dia { data: string; dia: number; estado: Estado; oracamentoAntes: number; oracamentoDepois: number; ultrapassagem: number; dentroOrcamento: boolean; ocupante: { clienteId: string; nome: string; valor: number } | null }
...
export function CicloLiberacaoFinanceiraV2() {
  ...
  const [modalCliente,setModalCliente] = useState<ClienteAgenda|null>(null);
  ...
  function clicarDia(x:Dia) {
    ...
    if (x.estado === "vermelho" && x.ocupante) {
      const cliente = clientes.find(c => c.clienteId === x.ocupante!.clienteId);
      if (cliente) setModalCliente(cliente);
      return;
    }
    if (selecionado) {
  ...
  return <div className="animate-fadeUp flex flex-col gap-3 pb-6">
    ...
    <ModalClienteLiberacaoFinanceira cliente={modalCliente ? { clienteId: modalCliente.clienteId, nome: modalCliente.nome, dataTermos: modalCliente.dataTermos, previsaoAtual: modalCliente.previsaoAtual, valor: modalCliente.valor, statusFinanceiro: modalCliente.statusFinanceiro, saldoRestante: modalCliente.saldoRestante } : null} onClose={()=>setModalCliente(null)} />
  </div>;
}
