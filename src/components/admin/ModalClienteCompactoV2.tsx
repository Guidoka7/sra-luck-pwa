...
import { AlertCircle, CalendarDays, Check, FileText, IdCard, Paperclip, Receipt, Trash2, UploadCloud, UserRound, WalletCards, X } from "lucide-react";
...
export function ModalClienteCompactoV2({ cliente, onClose, onSalvo }: { cliente: Cliente | null; onClose: () => void; onSalvo: () => void }) {
  ...
  const [inputFiles, setInputFiles] = useState<Record<string, HTMLInputElement | null>>({});
  ...
  async function rejeitarComprovante(boleto: Boleto) {
    ...
    try {
      const res = await fetch(`/api/admin/boletos/${boleto.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ acao: "rejeitar", observacoes: "Comprovante rejeitado pelo administrador." }) });
      const data = await res.json();
    ...
    } catch (e) { toast.error(e instanceof Error ? e.message : "Erro ao rejeitar comprovante."); }
  ...
  return <Portal><div className="fixed inset-0 z-40 flex items-center justify-center bg-burgundy-dark/55 px-2 py-2.5 backdrop-blur-md sm:px-4 sm:py-4"><div className="flex max-h-[94vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-white/10 bg-[#1b181b] text-pearl shadow-2xl">
    ...
    <footer className="flex shrink-0 items-center justify-between border-t border-white/10 px-3.5 py-2.5 sm:px-4"><div className="text-[0.55rem] uppercase tracking-[0.14em] text-pearl/25">Financeiro e comprovantes</div><div className="flex items-center gap-2"><Button type="button" variant="secondary" size="sm" onClick={onClose}>Fechar</Button>{aba === "dados" && <Button type="submit" form="form-cliente-v2" disabled={salvando} size="sm"><Check className="h-3.5 w-3.5" /> {salvando ? "Salvando" : "Salvar"}</Button>}</div></footer>
  </div></div></Portal>;
}
