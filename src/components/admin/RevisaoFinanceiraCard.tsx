...
export function RevisaoFinanceiraCard() {
  ...
  return <div className="space-y-3">
    <Panel className="p-4 sm:p-5">
      <SectionHeading title="Confirmação do levantamento financeiro" description="Confirme o saldo e o custeio para liberar a agenda." aside={<StatusPill tone={pendentes.length > 0 ? "rose" : "neutral"}>{pendentes.length} pendente(s)</StatusPill>} />
      {pendentes.length === 0 ? <div className="flex flex-col items-center gap-2 rounded-2xl border border-dashed border-rose/20 bg-blush/30 px-5 py-7 text-center"><ShieldCheck className="h-5 w-5 text-clay/30" /><p className="text-sm text-clay/55">Nenhuma cliente aguardando confirmação financeira.</p></div> : <div className="space-y-3">{pendentes.map((c) => { const config = financeiro[c.id] ?? { saldo: String(c.saldoRestanteEstimado ?? 0), taxa: "5.4", formas: ["cartao", "pix", "cheques", "boleto_100"] as Forma[] }; const dias = diasUteisDesde(c.dataAtingiuPercentual); const atrasado = dias >= PRAZO_REVISAO_FINANCEIRA_DIAS_UTEIS; return <div key={c.id} className="rounded-2xl border border-rose/10 bg-blush/30 p-3">
        <div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-semibold text-burgundy">{c.nome}</p><p className="text-[0.68rem] text-clay/50">{formatarMoeda(c.valorContrato)} · {c.quantidadeParcelas ?? "—"} parcelas · {c.porcentagemPagamento}% pago</p></div><p className={`flex items-center gap-1 text-[0.68rem] ${atrasado ? "text-alert" : "text-burgundy/60"}`}><Clock className="h-3 w-3" />{atrasado ? "Prazo atingido" : `${dias}/${PRAZO_REVISAO_FINANCEIRA_DIAS_UTEIS} dias úteis`}</p></div>
        <div className="mt-2 grid gap-2 sm:grid-cols-[1fr_0.5fr]"><label className="text-[0.58rem] font-semibold uppercase tracking-[0.16em] text-clay/45">Saldo confirmado<input value={config.saldo} onChange={(e) => alterarFinanceiro(c.id, { saldo: e.target.value })} type="number" min="0" step="0.01" className="mt-1 w-full rounded-lg border border-rose/15 bg-white px-2.5 py-2 text-sm text-burgundy outline-none focus:border-gold" /></label><label className="text-[0.58rem] font-semibold uppercase tracking-[0.16em] text-clay/45">Taxa cartão (%)<input value={config.taxa} onChange={(e) => alterarFinanceiro(c.id, { taxa: e.target.value })} type="number" min="0" step="0.1" className="mt-1 w-full rounded-lg border border-rose/15 bg-white px-2.5 py-2 text-sm text-burgundy outline-none focus:border-gold" /></label></div>
        <div className="mt-2"><p className="mb-1 text-[0.58rem] font-semibold uppercase tracking-[0.16em] text-clay/45">Formas de custeio</p><div className="grid gap-1.5 sm:grid-cols-4">{FORMAS.map((f) => <button type="button" key={f.value} onClick={() => alternarForma(c.id, f.value)} className={`flex min-h-10 items-center gap-1.5 rounded-lg border px-2 py-1.5 text-left transition ${config.formas.includes(f.value) ? "border-burgundy bg-burgundy/[0.06]" : "border-rose/10 bg-white"}`}><span className={`flex h-6 w-6 flex-none items-center justify-center rounded-full ${config.formas.includes(f.value) ? "bg-burgundy text-cream" : "bg-blush text-burgundy"}`}>{f.value === "cartao" ? <CreditCard className="h-3 w-3" /> : f.value === "pix" ? <Landmark className="h-3 w-3" /> : <FileCheck2 className="h-3 w-3" />}</span><span className="min-w-0"><span className="block truncate text-[0.68rem] font-semibold text-burgundy">{f.label}</span><span className="block truncate text-[0.56rem] text-clay/45">{f.description}</span></span></button>)}</div></div>
        <div className="mt-2 flex flex-wrap items-center justify-between gap-2 rounded-lg bg-white/70 p-2"><p className="text-[0.68rem] text-clay/55">Cartão com taxa: <strong className="text-burgundy">{formatarMoeda(Number(config.saldo || 0) * (1 + Number(config.taxa || 0) / 100))}</strong></p><div className="flex items-center gap-1.5"><Button size="sm" variant="secondary" loading={processando === c.id} onClick={() => decidir(c.id, "recusada")} className="!text-alert"><X className="h-3 w-3" /> Recusar</Button><Button size="sm" loading={processando === c.id} onClick={() => decidir(c.id, "aprovada")}><Check className="h-3 w-3" /> Confirmar e liberar</Button></div></div>
      </div>; })}</div>}
    </Panel>

    <Panel className="p-4 sm:p-5">
      <SectionHeading title="Solicitações de liberação financeira" description="Programe a liberação após a escolha dos termos." aside={<StatusPill tone={solicitacoes.length > 0 ? "rose" : "neutral"}>{solicitacoes.length}</StatusPill>} />
      {solicitacoes.length === 0 ? <div className="flex items-center justify-center rounded-2xl border border-dashed border-rose/20 bg-blush/30 px-5 py-6 text-center"><p className="text-sm text-clay/55">Nenhuma solicitação de custeio aguardando análise.</p></div> : <div className="space-y-2">{solicitacoes.map((s) => { const forma = FORMAS.find((f) => f.value === s.forma_custeio); const dataSugestao = s.previsao_sugerida ?? data90Dias(s.data_termos); const dataAtual = datasLiberacao[s.id] ?? dataSugestao ?? ""; return <div key={s.id} className="rounded-2xl border border-rose/10 bg-blush/30 p-3"><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-sm font-semibold text-burgundy">{s.clientes?.nome_completo ?? "Cliente"}</p><p className="text-[0.68rem] text-clay/45">{forma?.label ?? s.forma_custeio} · {s.agendamento_id ? `termos em ${s.data_termos ? new Date(`${s.data_termos}T12:00:00`).toLocaleDateString("pt-BR") : "data definida"}` : "aguardando data dos termos"}</p><p className="mt-0.5 text-[0.62rem] text-clay/50">Saldo: {formatarMoeda(Number(s.saldo_restante))}{s.forma_custeio === "cartao" ? ` · total com taxa: ${formatarMoeda(Number(s.total_com_taxa))}` : ""}</p></div><StatusPill tone={s.status === "pendente" ? "rose" : "neutral"}>{s.status === "pendente" ? "Nova" : s.status === "em_analise" ? "Em análise" : "Aprovada"}</StatusPill></div>
        <div className="mt-2 flex flex-wrap gap-1.5"><Button size="sm" variant="secondary" loading={processando === s.id} onClick={() => atualizarSolicitacao(s.id, "em_analise")}><Clock className="h-3 w-3" /> Em análise</Button><Button size="sm" variant="secondary" loading={processando === s.id} onClick={() => atualizarSolicitacao(s.id, "recusada")} className="!text-alert"><X className="h-3 w-3" /> Recusar</Button>{!s.agendamento_id ? <span className="self-center text-[0.68rem] text-clay/45">Aguardando a cliente escolher a data dos termos.</span> : <div className="flex flex-wrap items-center gap-1.5 rounded-lg bg-white/80 p-1"><input type="date" value={dataAtual} min={s.data_termos ?? undefined} onChange={(e) => setDatasLiberacao((atual) => ({ ...atual, [s.id]: e.target.value }))} className="rounded-md border border-rose/15 bg-white px-2 py-1.5 text-xs text-burgundy outline-none focus:border-gold" /><Button size="sm" loading={processando === s.id} onClick={() => programar(s.id)}><Check className="h-3 w-3" /> Programar</Button>{dataSugestao && <span className="text-[0.58rem] text-clay/45">Sugestão: 90 dias</span>}</div>}</div>
        {s.observacao && <p className="mt-2 rounded-lg bg-gold/[0.07] p-2 text-[0.68rem] leading-relaxed text-clay/60">{s.observacao}</p>}
      </div>; })}</div>}
    </Panel>
  </div>;
}
