            ...
            <div className="mt-2 overflow-hidden rounded-lg border border-white/8">
              {boletos.map((b) => <div key={b.id} className="grid grid-cols-[46px_1fr_auto] items-center gap-2 border-b border-white/6 px-2.5 py-2 last:border-0">
                ...
                <div className="flex shrink-0 items-center gap-1.5">
                  ...
                  <button type="button" disabled={processando === b.id} onClick={() => decidirParcela(b, "confirmar")} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-success/30 bg-success/15 text-success shadow-sm transition hover:bg-success/25 disabled:opacity-40" title="Confirmar parcela" aria-label={`Confirmar parcela ${b.numero_parcela}`}><Check className="h-4 w-4" strokeWidth={2.5} /></button>
                  <button type="button" disabled={processando === b.id} onClick={() => decidirParcela(b, "rejeitar")} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-alert/30 bg-alert/15 text-alert shadow-sm transition hover:bg-alert/25 disabled:opacity-40" title="Recusar parcela" aria-label={`Recusar parcela ${b.numero_parcela}`}><X className="h-4 w-4" strokeWidth={2.5} /></button>
                </div>
              </div>)}
            </div>
          </section>
