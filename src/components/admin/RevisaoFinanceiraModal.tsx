        ...
        {carregando ? <div className="p-10 text-center text-xs text-pearl/40">Carregando dados, parcelas e comprovantes…</div> : <div className="space-y-3">
          ...
          <section className="rounded-xl border border-white/8 bg-white/[0.025] p-3">
            ...
            <div className="mt-2 overflow-hidden rounded-lg border border-white/8">
              {boletos.map((b) => {
                const confirmada = b.status === "pago";
                const recusada = !confirmada && String(b.observacoes ?? "").toLowerCase().includes("recus");
                return <div key={b.id} className={`grid grid-cols-[46px_1fr_auto] items-center gap-2 border-b px-2.5 py-2 last:border-0 transition-colors ${confirmada ? "border-success/15 bg-success/[0.075]" : recusada ? "border-alert/20 bg-alert/[0.075]" : "border-white/6 bg-transparent"}`}>
                  <div className="flex items-center gap-1.5"><span className={`h-2 w-2 rounded-full ${confirmada ? "bg-success" : recusada ? "bg-alert" : "bg-gold/45"}`} /><span className={`text-[0.6rem] font-semibold ${confirmada ? "text-success" : recusada ? "text-alert" : "text-pearl/60"}`}>{b.numero_parcela}/{b.total_parcelas}</span></div>
                  <div className="min-w-0"><div className="flex flex-wrap gap-x-2 text-[0.6rem]"><span className={confirmada ? "text-success/75" : recusada ? "text-alert/75" : "text-pearl/45"}>{b.data_vencimento?.split("-").reverse().join("/") ?? "—"}</span><strong className={confirmada ? "text-success" : recusada ? "text-alert" : "text-pearl/85"}>{formatarMoeda(Number(b.valor))}</strong></div><p className={`mt-0.5 text-[0.5rem] ${confirmada ? "text-success" : recusada ? "text-alert" : "text-pearl/35"}`}>{confirmada ? "Pagamento confirmado" : recusada ? "Pagamento recusado · precisa de nova conferência" : "Em aberto"}{b.comprovante_url ? " · comprovante anexado" : " · sem comprovante"}</p></div>
                  <div className="flex shrink-0 items-center gap-1.5">
                    {b.comprovante_url && <a href={`/api/admin/boletos/${b.id}/comprovante`} target="_blank" rel="noopener noreferrer" className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-rose/20 bg-rose/10 text-rose transition hover:bg-rose/20" title="Abrir comprovante" aria-label="Abrir comprovante"><FileText className="h-3.5 w-3.5" /></a>}
                    <button type="button" disabled={processando === b.id} onClick={() => decidirParcela(b, "confirmar")} className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full border shadow-sm transition ${confirmada ? "border-success/50 bg-success text-white ring-2 ring-success/15" : "border-success/30 bg-success/15 text-success hover:bg-success/25"} disabled:opacity-40`} title="Confirmar parcela" aria-label={`Confirmar parcela ${b.numero_parcela}`}><Check className="h-4 w-4" strokeWidth={2.5} /></button>
                    <button type="button" disabled={processando === b.id} onClick={() => decidirParcela(b, "rejeitar")} className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full border shadow-sm transition ${recusada ? "border-alert/50 bg-alert text-white ring-2 ring-alert/15" : "border-alert/30 bg-alert/15 text-alert hover:bg-alert/25"} disabled:opacity-40`} title="Recusar parcela" aria-label={`Recusar parcela ${b.numero_parcela}`}><X className="h-4 w-4" strokeWidth={2.5} /></button>
                  </div>
                </div>;
              })}
            </div>
