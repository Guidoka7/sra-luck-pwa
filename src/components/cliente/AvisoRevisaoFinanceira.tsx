  ...
  useEffect(() => {
    ...
    async function carregarDatas() {
      try {
        ...
        if (ativo) setDatas(data.datasDisponiveis ?? []);
      } catch {}
    }
    void carregarDatas();
    const intervalo = setInterval(() => void carregarDatas(), 10000);
        ...
        <div className="flex items-start gap-3 p-3.5 sm:p-4">
          ...
          <div className="min-w-0">
            ...
            <h2 className="mt-1 font-heading text-sm font-semibold leading-tight text-burgundy">
              {recusada ? "Precisamos regularizar uma divergência antes da liberação" : "Sua agenda de assinatura está bloqueada temporariamente"}
            </h2>
            <p className="mt-1.5 text-[0.72rem] leading-relaxed text-clay/65">
              {recusada
                ...
                : "A agenda continua visível para a senhora, porém bloqueada enquanto nossa equipe confere os pagamentos e comprovantes enviados. Assim que o levantamento for concluído, a escolha da data será liberada normalmente."
              }
            </p>
            ...
            {observacao && recusada && <p className="mt-2 rounded-lg bg-alert/8 px-2.5 py-2 text-[0.62rem] leading-relaxed text-alert">{observacao}</p>}
          </div>
        </div>
      </section>
      ...
      <section className="overflow-hidden rounded-2xl border border-rose/15 bg-white/70 p-3 opacity-75 shadow-[0_14px_40px_-28px_rgba(0,0,0,.25)] dark:border-white/10 dark:bg-white/[0.035] sm:p-4">
        <div className="mb-3 flex items-center justify-between gap-2">
          <div>
            ...
            <h3 className="mt-0.5 font-heading text-sm font-semibold text-burgundy dark:text-cream">Escolha da data bloqueada</h3>
          </div>
          <span className="rounded-full border border-gold/20 bg-gold/10 px-2.5 py-1 text-[0.54rem] font-bold uppercase tracking-[0.08em] text-gold">Bloqueada</span>
        </div>
