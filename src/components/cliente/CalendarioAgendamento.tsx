...
import { addMonths, eachDayOfInterval, endOfMonth, endOfWeek, format, isBefore, isSameMonth, isToday, startOfDay, startOfMonth, startOfWeek, subMonths } from "date-fns";
...
export function CalendarioAgendamento({ datas, onConfirmar, confirmando, bloqueado = false }: CalendarioAgendamentoProps) {
  ...
  const dias = useMemo(() => { const inicio = startOfWeek(startOfMonth(mesAtual), { weekStartsOn: 0 }); const fim = endOfWeek(endOfMonth(mesAtual), { weekStartsOn: 0 }); return eachDayOfInterval({ start: inicio, end: fim }); }, [mesAtual]);
  ...
  function mudarMes(delta: 1 | -1) { setDirecao(delta); setMesAtual((atual) => (delta === 1 ? addMonths(atual, 1) : subMonths(atual, 1))); }
  ...
  const prazoLevantamento = dataAtingiuPercentual ? adicionarDiasUteis(new Date(dataAtingiuPercentual), 5) : null;
  ...
  return (
    <div className="relative">
      ...
      <div className={bloqueado ? "pointer-events-none select-none blur-[2px]" : ""}>
        ...
        <AnimatePresence mode="wait"><motion.div key={format(mesAtual, "yyyy-MM")} initial={{ opacity: 0, x: direcao === 1 ? 16 : -16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: direcao === 1 ? -16 : 16 }} transition={{ duration: 0.28, ease: [0.22, 1, 0.36, 1] }} className="mx-auto grid max-w-[22rem] grid-cols-7 gap-1 sm:max-w-[24rem] sm:gap-1.5">
          {dias.map((dia, i) => { const noMes = isSameMonth(dia, mesAtual); const estado = estadoDoDia(dia); const chave = format(dia, "yyyy-MM-dd"); const selecionado = chave === diaSelecionado; const ehHoje = isToday(dia); return <button key={dia.toISOString()} onClick={() => selecionarDia(dia, estado)} disabled={estado !== "disponivel"} title={estado === "lotado" ? "Agenda lotada" : undefined} style={{ animationDelay: `${i * 8}ms` }} className={cn("group relative aspect-square animate-fadeIn rounded-xl border text-[0.78rem] transition-all duration-200", !noMes && "opacity-0 pointer-events-none", estado === "passado" && "border-transparent text-clay/20", estado === "lotado" && "cursor-not-allowed border-transparent bg-alert/[0.06] text-alert/45 line-through decoration-alert/30", estado === "disponivel" && !selecionado && "border-success/35 bg-success/10 font-medium text-success shadow-sm hover:-translate-y-0.5 hover:border-success/55 hover:bg-success/20 hover:shadow-card cursor-pointer", selecionado && "animate-popIn border-2 border-gold bg-burgundy font-semibold text-cream shadow-soft cursor-pointer")}><span className={cn("flex h-full w-full flex-col items-center justify-center gap-0.5", ehHoje && !selecionado && "ring-2 ring-rose/60 ring-inset rounded-xl")}><span className={cn(ehHoje && !selecionado && "text-rose font-bold")}>{format(dia, "d")}</span></span>{estado === "lotado" && <span className="pointer-events-none absolute -top-1.5 left-1/2 z-10 hidden -translate-x-1/2 -translate-y-full whitespace-nowrap rounded-md bg-burgundy-dark px-2 py-1 text-[0.6rem] font-medium normal-case text-cream shadow-soft group-hover:block">Agenda lotada</span>}</button>; })}
