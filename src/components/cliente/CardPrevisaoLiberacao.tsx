...
export function CardPrevisaoLiberacao({ previsaoLiberacaoFinanceira, dataAssinatura, destacar = false }: CardPrevisaoLiberacaoProps) {
  ...
  return <div style={{ perspective: 1200 }}><AnimatePresence mode="wait"><motion.div key={`${estado}-${previsaoLiberacaoFinanceira ?? "sem-cirurgia"}`} initial={{ opacity: 0, rotateY: -16, scale: 0.98 }} animate={{ opacity: 1, rotateY: 0, scale: 1 }} exit={{ opacity: 0, rotateY: 16, scale: 0.98 }} transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }} style={{ transformStyle: "preserve-3d" }}>
