...
export function SolicitarLiberacaoFinanceira({ ativo = true }: Props) {
  ...
  async function solicitarAlteracaoTermos(dataId: string, horario: string) {
    ...
    try {
      ...
      setAlteracao(null); setAlteracaoPendente(true);
  ...
  function abrirAlteracao(tipo: Exclude<TipoAlteracao, null>) {
    setErro(null); setAlteracaoPendente(false); setAlteracao(tipo);
  }
  ...
  const renderDataCard = (titulo: string, iso: string, tipo: Exclude<TipoAlteracao, null>) => {
    ...
    return <div className="group relative flex min-h-[150px] flex-col justify-between rounded-2xl border border-rose/12 bg-white px-5 py-5 shadow-[0_12px_35px_-28px_rgba(82,28,42,.38)] dark:border-white/8 dark:bg-white/[0.025]">
      <div className="flex items-start justify-between gap-3"><div><p className="text-[0.54rem] font-semibold uppercase tracking-[0.2em] text-clay/55 dark:text-pearl/60">{titulo}</p><div className="mt-2 flex items-end gap-2"><span className="font-heading text-[2.55rem] font-semibold leading-none text-burgundy dark:text-cream">{data.dia}</span><span className="pb-0.5 text-[0.72rem] font-semibold uppercase tracking-[0.12em] text-rose">{data.mes}</span></div><p className="mt-1 text-[0.62rem] font-medium tracking-[0.12em] text-clay/55 dark:text-pearl/60">{data.ano}</p></div><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-rose/[0.06] text-rose"><CalendarDays className="h-[17px] w-[17px]" /></span></div>
  ...
  return <div className="flex flex-col gap-4">
    {dataAssinaturaTermos && dataCirurgia && <section className="overflow-hidden rounded-2xl border border-rose/15 bg-white/75 shadow-[0_14px_40px_-28px_rgba(0,0,0,.3)] dark:border-white/10 dark:bg-white/[0.035]"><div className="flex items-center justify-between gap-3 border-b border-rose/10 px-4 py-3.5"><div><p className="text-[0.55rem] font-semibold uppercase tracking-[0.18em] text-rose">Minha agenda</p><p className="mt-0.5 text-xs text-clay/60 dark:text-pearl/60">Datas registradas para o seu atendimento.</p></div><span className="rounded-full border border-success/15 bg-success/[0.06] px-2.5 py-1 text-[0.5rem] font-bold uppercase tracking-[0.12em] text-success">Confirmada</span></div><div className="grid gap-3 p-3 sm:grid-cols-2 sm:p-4">{renderDataCard("Assinatura dos termos", dataAssinaturaTermos, "termos")}{renderDataCard("Data da sua cirurgia", dataCirurgia, "cirurgia")}</div></section>}

    {alteracaoPendente && <div className="rounded-2xl border border-gold/25 bg-gold/[0.07] px-4 py-3 text-sm leading-relaxed text-burgundy dark:text-cream">Sua solicitação de alteração está em análise no painel administrativo. Prazo de análise: <strong>até 5 dias úteis</strong>. Sua agenda atual permanece inalterada até a confirmação.</div>}

    {alteracao === "termos" && <section className="overflow-hidden rounded-2xl border border-rose/15 bg-white/75 p-3 shadow-card dark:border-white/10 dark:bg-white/[0.035]"><div className="mb-3 flex items-center justify-between"><div><p className="text-[0.58rem] font-bold uppercase tracking-label text-rose">Alteração de data</p><h3 className="text-sm font-semibold text-burgundy dark:text-cream">Escolha uma nova data para assinatura dos termos</h3></div><button type="button" onClick={() => setAlteracao(null)} className="rounded-full p-1 text-clay/70 dark:text-pearl/70"><X className="h-4 w-4" /></button></div><CalendarioAgendamento datas={datasTermos} onConfirmar={solicitarAlteracaoTermos} confirmando={enviando} /></section>}

    {alteracao === "cirurgia" && dataAssinaturaTermos && <section className="overflow-hidden rounded-2xl border border-rose/15 bg-white/75 p-3 shadow-card dark:border-white/10 dark:bg-white/[0.035]"><div className="mb-3 flex items-center justify-between"><div><p className="text-[0.58rem] font-bold uppercase tracking-label text-rose">Alteração de data</p><h3 className="text-sm font-semibold text-burgundy dark:text-cream">Escolha uma nova data para sua cirurgia</h3></div><button type="button" onClick={() => setAlteracao(null)} className="rounded-full p-1 text-clay/70 dark:text-pearl/70"><X className="h-4 w-4" /></button></div><CalendarioCirurgia dataAssinatura={dataAssinaturaTermos} dataCirurgiaAtual={dataCirurgia} onConfirmada={(data) => { setDataCirurgia(data); setAlteracao(null); }} /></section>}

    {dataAssinaturaTermos && !dataCirurgia && !alteracao && <section className={cn("rounded-2xl border px-4 py-3", recusada ? "border-alert/20 bg-alert/[0.05]" : "border-success/20 bg-success/[0.05]")}><div className="flex items-center gap-3"><CheckCircle2 className={cn("h-5 w-5", recusada ? "text-alert" : "text-success")} /><div className="flex-1"><p className="text-[0.58rem] font-semibold uppercase tracking-label text-success">{recusada ? "Custeio precisa de revisão" : aprovada ? "Custeio confirmado" : "Próxima etapa"}</p><p className="text-sm text-burgundy dark:text-cream">Informe como será realizado o pagamento do saldo restante.</p></div>{(!solicitacao || recusada) && <button onClick={() => { setErro(null); setModalAberto(true); }} className="rounded-lg bg-rose px-3 py-2 text-[0.58rem] font-bold uppercase text-white"><CreditCard className="mr-1 inline h-3 w-3" /> Informar</button>}</div></section>}
