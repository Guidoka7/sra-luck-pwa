...
export function SolicitarLiberacaoFinanceira({ ativo = true }: Props) {
  ...
  return <div className="flex flex-col gap-4">
    ...
    {solicitacao && dataCirurgia && dataAssinaturaTermos && <section className="overflow-hidden rounded-2xl border border-rose/15 bg-white/80 p-3 shadow-[0_14px_40px_-28px_rgba(0,0,0,.35)] dark:border-white/10 dark:bg-white/[0.035] sm:p-4"><div className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-success" /><p className="text-[0.58rem] font-bold uppercase tracking-[0.14em] text-success">Datas confirmadas</p></div><div className="mt-3 grid grid-cols-2 gap-2.5"><div className="rounded-2xl border border-rose/12 bg-blush/45 p-3 text-center"><p className="text-[0.55rem] font-bold uppercase tracking-label text-clay/45">Assinatura dos termos</p><p className="mt-1.5 text-lg font-heading font-bold text-burgundy">{dataAssinaturaTermos.split("-").reverse().join("/")}</p></div><div className="rounded-2xl border border-rose/20 bg-rose/[0.06] p-3 text-center"><p className="text-[0.55rem] font-bold uppercase tracking-label text-clay/45">Data da sua cirurgia</p><p className="mt-1.5 text-lg font-heading font-bold text-burgundy">{dataCirurgia.split("-").reverse().join("/")}</p></div></div></section>}
