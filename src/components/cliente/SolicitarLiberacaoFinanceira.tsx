  ...
  return <>
    ...
    <AnimatePresence>
      {modalAberto && <motion.div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/55 p-4 backdrop-blur-sm" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onMouseDown={e=>{if(e.target===e.currentTarget&&!enviando)setModalAberto(false)}}>
        <motion.div role="dialog" aria-modal="true" aria-labelledby="solicitar-liberacao-titulo" initial={{opacity:0,y:12,scale:.98}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:12,scale:.98}} transition={{duration:.2}} className="w-full max-w-md overflow-hidden rounded-2xl border border-rose/15 bg-white p-4 shadow-2xl dark:bg-[#1b191b] sm:p-5">
          <div className="flex items-start justify-between gap-3"><div><p className="text-[0.6rem] font-semibold uppercase tracking-label text-rose">Liberação financeira</p><h2 id="solicitar-liberacao-titulo" className="mt-1 font-heading text-lg font-semibold text-burgundy">Escolha o custeio</h2></div><button type="button" onClick={()=>!enviando&&setModalAberto(false)} className="rounded-full p-1.5 text-clay/50 transition hover:bg-blush hover:text-burgundy" aria-label="Fechar"><X className="h-4 w-4" /></button></div>
          <div className="mt-3 grid grid-cols-2 gap-2 rounded-xl bg-blush/45 p-3 dark:bg-white/[0.045]"><div><p className="text-[0.55rem] font-semibold uppercase tracking-label text-clay/45">Saldo restante</p><p className="mt-0.5 text-sm font-bold text-burgundy">{formatarMoeda(saldoRestante)}</p></div><div><p className="text-[0.55rem] font-semibold uppercase tracking-label text-clay/45">Cartão com taxa</p><p className="mt-0.5 text-sm font-bold text-burgundy">{formatarMoeda(totalCartao)}</p></div></div>
          <div className="mt-3 grid gap-2">{formasDisponiveis.map(forma=><button key={forma.id} type="button" onClick={()=>setFormaCusteio(forma.id)} className={cn("flex items-center justify-between gap-3 rounded-xl border px-3 py-2.5 text-left transition",formaCusteio===forma.id?"border-burgundy bg-burgundy text-cream":"border-rose/15 bg-white text-clay/70 hover:border-rose dark:bg-white/[0.035]")}><span className="min-w-0"><span className={cn("block text-[0.72rem] font-semibold",formaCusteio===forma.id?"text-cream":"text-burgundy")}>{forma.titulo}</span><span className={cn("block text-[0.6rem]",formaCusteio===forma.id?"text-cream/70":"text-clay/50")}>{forma.descricao}</span></span><span className={cn("h-3.5 w-3.5 shrink-0 rounded-full border",formaCusteio===forma.id?"border-cream bg-cream":"border-clay/25")} /></button>)}</div>
