...
export function SolicitarLiberacaoFinanceira({ ativo = true }: Props) {
  ...
  return <div className="flex flex-col gap-4">
    <section className={cn("overflow-hidden rounded-2xl border shadow-[0_14px_40px_-28px_rgba(0,0,0,.35)]", recusada ? "border-alert/15 bg-alert/[0.045]" : "border-success/15 bg-success/[0.045]")}>
      <div className="flex items-center gap-2.5 px-3 py-3 sm:px-3.5 sm:py-3.5">
        <span className={cn("flex h-8 w-8 shrink-0 items-center justify-center rounded-lg", recusada ? "bg-alert/10 text-alert" : "bg-success/10 text-success")}><CheckCircle2 className="h-4 w-4" /></span>
        <div className="min-w-0 flex-1">
          <p className={cn("text-[0.58rem] font-semibold uppercase tracking-[0.14em]", recusada ? "text-alert" : "text-success")}>Financeiro confirmado</p>
          <h3 className="mt-0.5 font-heading text-sm font-semibold leading-tight text-burgundy">Agora escolha a data da sua cirurgia</h3>
          <p className="mt-0.5 text-[0.62rem] leading-[1.4] text-clay/55">A data da assinatura dos termos já foi escolhida. Para escolher a data da cirurgia, informe como será realizado o pagamento do saldo restante.</p>
        </div>
        {!solicitacao && <button type="button" onClick={abrirModal} className="inline-flex shrink-0 items-center justify-center gap-1.5 rounded-lg bg-rose px-2.5 py-2 text-[0.55rem] font-bold uppercase tracking-[0.08em] text-white shadow-card transition hover:opacity-90"><CreditCard className="h-3 w-3" /> Solicitar data da cirurgia</button>}
        {solicitacao && <span className={cn("hidden shrink-0 rounded-lg border px-2.5 py-1.5 text-[0.54rem] font-bold uppercase tracking-[0.08em] sm:inline-flex", recusada ? "border-alert/15 bg-alert/10 text-alert" : "border-success/15 bg-success/10 text-success")}>{recusada ? "Revisar custeio" : aprovada ? "Custeio confirmado" : "Custeio enviado"}</span>}
      </div>
    </section>
    ...
    {solicitacao && dataCirurgia && <section className="overflow-hidden rounded-2xl border border-rose/15 bg-white/80 p-3 shadow-[0_14px_40px_-28px_rgba(0,0,0,.35)] dark:border-white/10 dark:bg-white/[0.035] sm:p-4"><div className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-success" /><p className="text-[0.58rem] font-bold uppercase tracking-[0.14em] text-success">Datas confirmadas</p></div><div className="mt-3 grid grid-cols-2 gap-2.5"><div className="rounded-2xl border border-rose/12 bg-blush/45 p-3 text-center"><p className="text-[0.55rem] font-bold uppercase tracking-label text-clay/45">Assinatura dos termos</p><p className="mt-1.5 text-lg font-heading font-bold text-burgundy">{dataAssinaturaTermos.split("-").reverse().join("/")}</p></div><div className="rounded-2xl border border-rose/20 bg-rose/[0.06] p-3 text-center"><p className="text-[0.55rem] font-bold uppercase tracking-label text-clay/45">Data da sua cirurgia</p><p className="mt-1.5 text-lg font-heading font-bold text-burgundy">{dataCirurgia.split("-").reverse().join("/")}</p></div></div></section>}
